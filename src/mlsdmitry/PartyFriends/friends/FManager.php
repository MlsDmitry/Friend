...
class FManager implements Listener
{
    ...
    {
        $p = $event->getPlayer();
        if (!PF::$friends_db->exists(self::name($p))) {
            PF::$friends_db->set(self::name($p));
            PF::$friends_db->save();
        }
        if (!PF::$associations->exists(self::name($p))) {
            PF::$associations->setNested(self::name($p), []);
    ...
    {
        return in_array(self::name($p2), PF::$associations->getNested(self::name($p)));
    }
     ...
     * @return bool
     */
    private static function name($p_name)
    {
    ...
    public static function request(Player $requester, $awaiter)
    {
        ...
        $awaiter_obj = Server::getInstance()->getPlayer($awaiter);
        ...
        self::$requests[self::name($awaiter)][self::name($requester)] = $r;
        return self::SUCCESS;
    }
    ...
    public static function accept(Player $awaiter, string $requester)
    {
        ...
        if (!isset(self::$requests[self::name($awaiter)]))
            return self::DONT_HAVE_REQUESTS;

        if (!isset(self::$requests[self::name($awaiter)][self::name($requester)]))
            return self::INVALID_NAME;


        /** @var Request $r */
        $r = self::$requests[self::name($awaiter)][self::name($requester)];
    ...
    public static function add($p1, $p2)
    {
        ...
        $p1_friends = PF::$associations->getNested(self::name($p1));
        $p1_friends[] = self::name($p2);
        PF::$associations->setNested(self::name($p1), $p1_friends);

        $p2_friends = PF::$associations->getNested(self::name($p2));
        $p2_friends[] = self::name($p1);
        PF::$associations->setNested(self::name($p2), $p2_friends);
    ...
    public static function remove($p1, $p2)
    {
        ...
        if (in_array(self::name($p2), $p1_friends))
            unset($p1_friends[array_search(self::name($p1), $p1_friends)]);
        else
        ...
        if (in_array(self::name($p1), $p2_friends))
            unset($p2_friends[array_search(self::name($p2), $p2_friends)]);
    ...
    public static function deny($awaiter, $requester)
    {
        ...
        unset(self::$requests[self::name($awaiter)][self::name($requester)]);
        return self::SUCCESS;
    }
    ...
    {
        if (PF::$associations->getNested(self::name($p)) === [])
            return self::DONT_HAVE_FRIENDS;
        return PF::$associations->getNested(self::name($p));
    }
    ...
    {
        if (!isset(self::$requests[self::name($p)]))
            return self::DONT_HAVE_REQUESTS;
        return self::$requests[self::name($p)];
    }
}